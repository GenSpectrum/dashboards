...
function removeObserver(provider, context, consumer) {
    ...
    const consumerIndex = observers.findIndex((observer) => observer.consumer === consumer);
    if (consumerIndex !== -1) {
...
function onContextObserve(provider, context, callback) {
    const providedContext = getProvidedContext(provider, context, 'onContextObserve');
    const listeners = providedContext.listeners || (providedContext.listeners = []);
...
function onContextUnobserve(provider, context, callback) {
    const providedContext = getProvidedContext(provider, context, 'onContextUnobserve');
