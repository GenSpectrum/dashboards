...
export type VariantFilterConfig = {
    ...
    isInVariantQueryMode: boolean | null;
};
...
export function VariantSelector({
    ...
    const [isInVariantQueryMode, setIsInVariantQueryMode] = useState(variantFilterConfig.isInVariantQueryMode);
    useEffect(
        () => setIsInVariantQueryMode(variantFilterConfig.isInVariantQueryMode),
        [variantFilterConfig.isInVariantQueryMode],
    );

    return (
        <div>
            <label
                className={`mb-1 flex cursor-pointer items-center gap-1 text-sm ${isInVariantQueryMode === null ? 'hidden' : ''}`}
            >
                Advanced
                <input
                    type='checkbox'
                    ...
                    checked={isInVariantQueryMode ?? false}
                />
            </label>
